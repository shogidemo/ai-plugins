...
copilot --model gpt-5.4 --add-dir src -s -p "src/main.py をレビューしてください。
...
copilot --model gpt-5.4 -s -p "以下のファイル内容をレビューしてください。
...
# Step 2: レビュー実行
copilot --model gpt-5.4 --add-dir . -s -p "implementation_plan.md をレビューしてください。抜け漏れや改善点があれば指摘してください。
...
# Step 2: プロンプト内に埋め込んでレビュー
copilot --model gpt-5.4 -s -p "以下の設計書をレビューしてください。技術的な問題点や考慮漏れがあれば教えてください。
...
copilot --model gpt-5.4 --add-dir src -s -p "src/file.py をレビューしてください。"

# 全パスへのアクセスを許可（注意して使用、非推奨）
copilot --model gpt-5.4 --allow-all-paths -s -p "src/file.py をレビューしてください。"
```
...
1. **推奨: `--add-dir` を最小スコープで使用**
   ...
   # Step 2: レビュー実行
   copilot --model gpt-5.4 --add-dir path/to -s -p "path/to/file.md をレビューしてください。レビュー対象は指定されたファイルのみ。入力が見えない場合は『入力が確認できません』と回答してください。まず対象ファイル名を復唱してから開始してください。"
   ```
...
2. **代替: プロンプト内にファイル内容を埋め込む（小ファイル向け）**
   ...
   # Step 2: プロンプト内に埋め込んでレビュー
   copilot --model gpt-5.4 -s -p "以下をレビューしてください。レビュー対象は以下の内容のみ。入力が空の場合は『入力がありません』と回答してください。
...
- **レビュー依頼時は必ず `--model gpt-5.4 -s -p` を指定すること**

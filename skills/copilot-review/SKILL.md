...
1. **パスは絶対パスを直接記述する**（Claude は対象ファイルの絶対パスを知っている）
2. **`--add-dir` には親ディレクトリの絶対パスを直接記述する**
3. **ファイル内容の埋め込み（`$(cat ...)`）は使用しない** → `--add-dir` 方式を使用すること
4. **`test` と `copilot` は必ず別の Bash ツールコールとして実行する** — 1つのコマンドに `&&` や `||` で結合してはならない。`{ }` ブレース構文も使用しない
5. **変数代入と `copilot` コマンドを結合しない** — `FILE=... && copilot ...` のようなパターンは禁止
...
```bash
# --add-dir で最小スコープのディレクトリアクセスを許可
# パスは絶対パスを直接記述すること（$() コマンド置換は使用禁止）

# Step 1: ファイル存在確認
test -s /absolute/path/to/src/main.py

# Step 2: レビュー実行（--add-dir にはファイルの親ディレクトリの絶対パスを指定）
...
```bash
# --add-dir を最小スコープで使用 + CLIガード + 強化プロンプト
# パスは絶対パスを直接記述すること（$() コマンド置換は使用禁止）
...
1. **推奨: `--add-dir` を最小スコープで使用**（`$()` コマンド置換は使用しないこと）
   ```bash
   # Step 1: ファイル存在確認
   test -s /absolute/path/to/file.md

   # Step 2: レビュー実行（絶対パスを直接記述）

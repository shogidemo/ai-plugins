...
**`$()` コマンド置換の使用禁止**: copilot コマンド内に `$(pwd)`, `$(dirname ...)`, `$(cat ...)` 等のコマンド置換を含めてはならない。Claude Code が「Command contains $() command substitution」という安全性チェックを行い、毎回ユーザー確認を求めてしまうため。

- **パスは絶対パスを直接記述する**（Claude は対象ファイルの絶対パスを知っている）
- **`--add-dir` には親ディレクトリの絶対パスを直接記述する**
- **ファイル内容の埋め込み（`$(cat ...)`）は使用しない** → `--add-dir` 方式を使用すること
